...
int main() {
	...
	int validEclipses = 0;
	/*
	 * Data entry loop, asks for a file to read from and, if it opens correctly
	 * reads the data and checks the eclipses for validity
	 */
	while (true)
	{
		...
		while (inFile.good())
		{
			...
			if (stop == true)
			{
				...
				array.ReplaceAt(eclipse, repeatedIndex);
				continue;
			}
	...
	/*
	 * Data manipulation loop. Asks the user to either sort, search, output, or quit.
	 */
	while (true)
	{
		cout << "Type 'O' for output, 'S' for sort, 'F' for find, or 'Q' for quit" << endl;
		getline(cin, userInput);

		/*
		 * outputs the data to either a file specified by the user or the standard output
		 */
		if (userInput == "O")
		{
			string s;
		...
		/*
		 * Sorts the data. Asks the user what column to sort over and then calls the sort function
		 */
		if (userInput == "S")
		{
			...
			string s;
			getline(cin, s);
			sort(array, 0, array.getElementNum()-1, atoi(s.c_str()) - 1);
			sortedBy = atoi(s.c_str());
		}
		/*
		 * Searches the data. Asks the user for which column to search and for a value to search for.
		 * IF the user picks a numberic column, the value is checked to make sure that it is numeric.
		 * Then it calls the search function/.
		 */
		if (userInput == "F")
		{
			...
			if ((s == "1") || (s=="2") || (s == "3") || (s == "5") || (s == "7") || (s == "8") || (s == "9") || (s == "11") || (s == "12") || (s == "15") || (s == "16") || (s == "17"))
			{
				...
				if (valid == true)
				{
					...
					{
						search(outputArray, array, 0, array.getElementNum() - 1, atoi(s.c_str() - 1), value);
					}
					else
					{
						for (int i = 0; i < array.getElementNum(); i ++)
						{
							if (array.getData(i)->getData(atoi(s.c_str()) - 1) == value)
							{
								Eclipse* eclipse1;
								eclipse1 = array.getData(i);
								outputArray.Add(eclipse1);
