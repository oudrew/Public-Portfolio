...
void addHashTable(LinkedList<Eclipse>* eclipse, LinkedHashTable<Eclipse>* hashTable, int x)
{
	int id = atoi((eclipse->getData()->getData(0)).c_str());
	int key = id % hashTable->getTableSize();

	if (hashTable->isEmptyAt(key))
	{
		hashTable->setData(key, eclipse);
		hashTable->addKey(x, key);
	}
	else
	{
		addLinkedList(hashTable->getData(key), eclipse->getData());
	}
}
void addHashList(LinkedList<Eclipse>* hashList, LinkedList<Eclipse>* nextList)
{
	if (hashList->getNext() == 0)
	{
		hashList->setNext(nextList);
	}
	else
	{
		addHashList(hashList->getNext(), nextList);
	}
}

AVLNode<Eclipse>* minValueNode(AVLNode<Eclipse>* node)
		{
	AVLNode<Eclipse>* currentNode = node;
...
AVLNode<Eclipse>* insertAVLTree(AVLTree<Eclipse>* tree, AVLNode<Eclipse>* root, Eclipse* eclipse)
		{
	AVLNode<Eclipse>* newNode = new AVLNode<Eclipse>(*eclipse);
	if (root == NULL)
	{
		return newNode;
	}
	if (root->getData()->getData(0) == eclipse->getData(0))
	{
		root->setData(eclipse);
	}
	else if (atoi((eclipse->getData(0)).c_str()) < atoi((root->getData()->getData(0)).c_str()))
	{
		root->setLeft(insertAVLTree(tree, root->getLeft(), eclipse));
		root = tree->balance(root);
	}
	else if (atoi((eclipse->getData(0)).c_str()) >= atoi((root->getData()->getData(0)).c_str()))
	{
		root->setRight(insertAVLTree(tree, root->getRight(), eclipse));
		root = tree->balance(root);
	}
	return root;
		}

AVLNode<Eclipse>* deleteAVLTree(AVLTree<Eclipse>* tree, AVLNode<Eclipse>* root, Eclipse* eclipse)
		{
	if (root == NULL)
	{
		return root;
	...
	{
		root->setLeft(deleteAVLTree(tree, root->getLeft(), eclipse));
		root = tree->balance(root);
	}
	else if (atoi((eclipse->getData(0)).c_str()) > atoi((root->getData()->getData(0)).c_str()))
	{
		root->setRight(deleteAVLTree(tree, root->getRight(), eclipse));
		root = tree->balance(root);
	}
	else
	{
		AVLNode<Eclipse>* temp;
...
void preOrder(AVLNode<Eclipse>* root)
{
	if (root == NULL)
	{
		return;
...
void postOrder(AVLNode<Eclipse>* root)
{
	if (root == NULL)
	{
		return;
...
void inOrder(AVLNode<Eclipse>* root)
{
	if (root == NULL)
	{
		return;
...
void addInOrder(AVLNode<Eclipse>* root, ResizeableArray<Eclipse>* newArray)
{
	if (root == NULL)
	{
		return;
	}
	addInOrder(root->getLeft(), newArray);
	newArray->Add(root->getData());
	addInOrder(root->getRight(), newArray);
}
int main() {
	ifstream inFile;
	string file;
	...
	while (inFile.good())
	{
		...
		tree->setRoot(insertAVLTree(tree, tree->getRoot(), eclipse));
	...
	LinkedHashTable<Eclipse>* hashTablePointer;
	LinkedHashTable<Eclipse>* hashTable = new LinkedHashTable<Eclipse>(array.getElementNum());

	hashTablePointer = hashTable;

	for (int i = 0; i < array.getElementNum(); i ++)
	{
		LinkedList<Eclipse>* newList = new LinkedList<Eclipse>(*array.getData(i));
		LinkedList<Eclipse>* newList2 = new LinkedList<Eclipse>(*array.getData(i));
		addHashTable(newList, hashTable, i);
		addHashList(hashTable->getList(), newList2);
	}

	string userInput;
	while (true)
	{
		cout << "Type 'O' for output, 'S' for sort, 'F' for find, 'M' for merge, 'P' for purge, 'C' for catalog order, 'R' for pre-order \ntraversal, 'T' "
				"for post-order traversal, or 'Q' for quit" << endl;
		...
		if (userInput == "P")
		{
			...
			while (purgeFile.good())
			{
				...
				tree->setRoot(deleteAVLTree(tree, tree->getRoot(), eclipse1));
				x++;
			}
			ResizeableArray<Eclipse>* newArray = new ResizeableArray<Eclipse>();
			addInOrder(tree->getRoot(), newArray);
			delete arrayPointer;
		...
		if (userInput == "M")
		{
			...
			while (mergeFile.good())
			{
				...
				(*eclipse1).fixLeadingZero();
				tree->setRoot(insertAVLTree(tree, tree->getRoot(), eclipse1));
				x++;
			}
			ResizeableArray<Eclipse>* newArray = new ResizeableArray<Eclipse>();
