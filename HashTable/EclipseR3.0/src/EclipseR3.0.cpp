...
int main() {
	...
	while (true)
	{
		cout << "Type 'O' for output, 'S' for sort, 'F' for find, 'M' for merge, 'P' for purge, 'C' for catalog order, 'L' to print the table list,\n'H' to print the table, or 'Q' for quit" << endl;
		...
		if (userInput == "P")
		{
			...
			LinkedHashTable<Eclipse>* newHashTable = new LinkedHashTable<Eclipse>(arrayPointer->getElementNum());

			for (int i = 0; i < arrayPointer->getElementNum(); i ++)
			{
				LinkedList<Eclipse>* newList = new LinkedList<Eclipse>(*arrayPointer->getData(i));
				LinkedList<Eclipse>* newList2 = new LinkedList<Eclipse>(*arrayPointer->getData(i));
				addHashList(newHashTable->getList(), newList2);
				addHashTable(newList, newHashTable, i);
			}

			delete hashTablePointer;
		...
		if (userInput == "L")
		{
			...
			temp1 = hashTablePointer->getList()->getNext();
			temp = hashTablePointer->getList()->getNext();
		...
		if (userInput == "H")
		{
			for (int i = 0; i < hashTablePointer->getTableSize(); i ++)
			{
				cout << *(hashTablePointer->getData(i)->getData()) << endl;
				if (hashTablePointer->getData(i)->getNext() != 0)
				{
					cout << "Overflow: " << endl;
					LinkedList<Eclipse>* temp;
					LinkedList<Eclipse>* temp1;
					temp1 = hashTablePointer->getData(i)->getNext();
					temp = hashTablePointer->getData(i)->getNext();
					while (temp != 0)
					{
						temp1 = temp;
						cout << *temp->getData() << endl;
		...
		if (userInput == "F")
		{
			...
			if (s == "1")
			{
				...
				int key = id % hashTable->getTableSize();
				if (hashTable->getData(key)->getData()->getData(0) == value)
				{
					cout << *(hashTable->getData(key)->getData()) << endl;
				}
				else
				{
					if (hashTable->getData(key)->getNext() == 0)
					{
						cerr << "Eclipse not found" << endl;
					}
					else
					{
						LinkedList<Eclipse>* temp;
						LinkedList<Eclipse>* temp1;
						temp1 = hashTable->getData(key)->getNext();
						temp = hashTable->getData(key)->getNext();
						while (temp != 0)
						{
							temp1 = temp;
							if (temp->getData()->getData(0) == value)
							{
								cout << *temp->getData() << endl;
								break;
							}
			...
			if ((s=="2") || (s == "3") || (s == "5") || (s == "7") || (s == "8") || (s == "9") || (s == "11") || (s == "12") || (s == "15") || (s == "16") || (s == "17"))
			{
				cout << "Please select a value to search for" << endl;
				string value;
				bool valid = true;
